...
// Only want to make all the pieces once
...
function modelCopy() {
    ...
    for (let i = 0; i <= 7; i++) {
        result[i] = []
        for (let j = 0; j <= 7; j++) {
            result[i][j] = structuredClone(boardModel[i][j])
        }
    }
    ...
    for (let n = -1; n <= 1; n += 2) {
        for (let l = -1; l <= 1; l += 2) {
            ...
            if (occupier.includes(currentTurn)
                    ...
                    && y === (kingPosY + (currentTurn === "black" ? 1 : -1))))) {
                piecesAttacking.push([x, y])
            }
        }
    }
...
function moveRook(x, y) {
    ...
    if (!differenceXZero && differenceYZero) {
        ...
        for (let i = 1; i < differenceX * direction / 100; i++) {
            if (boardModel[(posX / 100) + i * direction][posY / 100].occupier !== "empty") {
                return false
            }
        }

        // Check if the selected piece is a rook first since this function is also called from the
        // moveQueen() function
        if (selectedPiece.type === "rook") {
            let colorStats = stats.get(currentTurn)
            
            if (selectedPiece.side === "kingside") {
        ...
        for (let i = 1; i < differenceY * direction / 100; i++) {
            if (boardModel[posX / 100][(posY / 100) + i * direction].occupier !== "empty") {
                return false
            }
        }
...
function moveKnight(x, y) {
    let differenceX = parseInt(x) - parseInt(selectedPiece.posX)
    let differenceY = parseInt(y) - parseInt(selectedPiece.posY)

    if ((Math.abs(differenceX) === 200 && Math.abs(differenceY) === 100) || 
            (Math.abs(differenceY) === 200 && Math.abs(differenceX) === 100)) {
        return movePiece(x, y)
    }
...
function moveBishop(x, y) {
    ...
    if (Math.abs(differenceX) === Math.abs(differenceY)) {
        ...
        for (let i = 1; i < differenceX * directionX / 100; i++) {
            if (boardModel[(posX / 100) + i * directionX][posY / 100 + i * directionY].occupier !== 
                    "empty") {
                return false
            }
        }
...
function moveQueen(x, y) {
    return moveBishop(x, y) || moveRook(x, y)
}
...
function moveKing(x, y) {
    ...
    let kingsideCastle = differenceYZero && (differenceX >= 200)
    let queensideCastle = differenceYZero && (differenceX <= -200)
...
let setCurrentTurn
const Board = ({ matchEnded, gameDrawn}) => {
    let squares = []
